...
> 更新：项目创建语义已经被 [0012 MVP 上线运营闭环 / 平台管理模型](./0012-mvp-launch-operability-and-admin-model.md) 覆盖。当前真值是：只有 `platform_admin` 可以创建项目，创建时必须指定首个项目 `admin`，平台管理员默认不自动拥有项目业务可见性。本文件中 Q2 的旧结论只作为历史背景保留。
