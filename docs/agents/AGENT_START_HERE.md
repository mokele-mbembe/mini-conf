...
- 前端管理台主链路已打通：登录、setup、首次改密、平台用户管理、平台项目创建、项目列表、配置文件、环境、部署实例、Draft、Saved Versions、preview、publish、Release detail/diff、实例归档/恢复/永久删除、项目成员管理、sync records。
...
- 当前仍缺：真实 staging 试部署反馈、heartbeats/audit logs 的真实前端页面、前端单元/组件测试基线、文档继续压缩。
...
4. 低风险运营页：heartbeats、audit logs。
