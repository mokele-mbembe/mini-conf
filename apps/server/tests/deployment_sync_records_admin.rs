...
use schema::{audit::DeploymentSyncRecordListResponse, project::ProjectSummary};
use support::{
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    r#"{"code":"sync-project","name":"Sync Project"}"#,
                ))?,
        )
        .await?;
    let project: ProjectSummary = read_json(response).await?;
    grant_project_role(&pool, project.id, "viewer-sync", "viewer").await?;
