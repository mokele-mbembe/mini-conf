...
async fn setup_app() -> TestResult<Option<(axum::Router, PgPool, String, String)>> {
    let Some(database_url) = test_database_url("open resolve") else {
        return Ok(None);
    };
...
#[tokio::test]
async fn resolve_returns_release_payload_and_etag() -> TestResult {
...
#[tokio::test]
async fn resolve_returns_unauthorized_when_bearer_token_is_missing() -> TestResult {
    let Some((app, pool, database_url, schema)) = setup_app().await? else {
        return Ok(());
    };
...
#[tokio::test]
async fn resolve_returns_not_modified_when_client_already_has_latest_revision() -> TestResult {
    let Some((app, pool, database_url, schema)) = setup_app().await? else {
        return Ok(());
    };
...
#[tokio::test]
async fn resolve_returns_config_file_not_found_when_config_is_missing() -> TestResult {
    let Some((app, pool, database_url, schema)) = setup_app().await? else {
        return Ok(());
    };
